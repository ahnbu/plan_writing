# CHANGELOG

모든 Git 커밋 이력을 최신순으로 기록합니다. 새 커밋은 표 최상단에 추가합니다.

| 일시 | 유형 | 범위 | 변경내용 (목적 포함) |
|---|---|---|---|
| 2026-07-08 11:27 | chore | gitignore | gitignore에 .codegraph/를 추가해 CodeGraph 로컬 캐시가 작업트리에 노출되지 않도록 보완 |
| 2026-03-25 | chore | .gitignore | _handoff/ 항목 제거 — handoff git 추적 복원 |
| 2026-03-19 | fix | .gitignore | *작업중*.txt 패턴 수정 — 파일명 중간에 위치한 패턴도 매치되도록 수정 |
| 2026-03-19 | docs | README, docs | README.md, docs/mbb_sop.md 업데이트 — 프로젝트 문서 개선 |
| 2026-03-19 | chore | .gitignore | _handoff/, 작업중*.txt 패턴 추가 — 불필요 파일 추적 방지 |
